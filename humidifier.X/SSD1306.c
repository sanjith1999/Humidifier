...
#define SSD1306_I2C_Start    I2C_Begin
#define SSD1306_I2C_Write    I2C_Write
#define SSD1306_I2C_Stop     I2C_End
...
void drawPixel(uint8_t x, uint8_t y, uint8_t color) {
    ...
    // x is which column
    switch (color) {
        case WHITE: ssd1306_buffer[x + (uint16_t) (y / 8) * SSD1306_LCDWIDTH] |= (1 << (y & 7));
            break;
        case BLACK: ssd1306_buffer[x + (uint16_t) (y / 8) * SSD1306_LCDWIDTH] &= ~(1 << (y & 7));
            break;
        case INVERSE: ssd1306_buffer[x + (uint16_t) (y / 8) * SSD1306_LCDWIDTH] ^= (1 << (y & 7));
            break;
    }
